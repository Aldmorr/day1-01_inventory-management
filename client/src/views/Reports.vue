    ...
    <div v-else>
      ...
      <div class="card">
        ...
        >
          <div class="bar-chart">
            <div class="y-axis">
              <span v-for="line in yAxisLabels" :key="line.value">${{ line.label }}</span>
            </div>
              ...
              <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
                <div class="bar-container">
                  <div
                    ...
                    :title="formatMonth(month.month) + ': $' + formatNumber(month.revenue)"
                    :aria-label="formatMonth(month.month) + ': $' + formatNumber(month.revenue)"
                  ></div>
                </div>
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">{{ t('reports.monthlyTrends.title') }}</h3>
        </div>
        <div class="table-container">
          <table class="reports-table">
            <thead>
              <tr>
                <th>{{ t('reports.monthlyTrends.month') }}</th>
                <th>{{ t('reports.monthlyTrends.orders') }}</th>
                <th>{{ t('reports.monthlyTrends.revenue') }}</th>
                <th>{{ t('reports.monthlyTrends.change') }}</th>
                <th>{{ t('reports.monthlyTrends.growthRate') }}</th>
              </tr>
            </thead>
            <tbody>
...
export default {
  ...
  setup() {
    ...
    const formatMonth = (monthStr) => {
      ...
      const monthName = monthKeys[idx] ? t(`months.${monthKeys[idx]}`) : month
    ...
    const bestQuarter = computed(() => {
      ...
      const best = quarterlyData.value.reduce((acc, q) =>
        q.total_revenue > acc.total_revenue ? q : acc
      )
      return best.quarter
    })
    ...
    const yAxisLabels = computed(() => {
      ...
      return steps.map(step => {
        ...
        return {
          value,
          label: value.toLocaleString(localeTag.value, {
    ...
    const chartAriaLabel = computed(() => {
      if (monthlyData.value.length === 0) return t('reports.monthlyChart.title')
      const first = monthlyData.value[0]
      const last = monthlyData.value[monthlyData.value.length - 1]
      return `${t('reports.monthlyChart.title')}: ${formatMonth(first.month)} $${formatNumber(first.revenue)} to ${formatMonth(last.month)} $${formatNumber(last.revenue)}`
    })
    ...
    watch([selectedPeriod, selectedLocation, selectedCategory, selectedStatus], () => {
      ...
      filterDebounce = setTimeout(loadData, 150)
    })
